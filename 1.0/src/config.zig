const rl = @import("raylib");
const DB16 = @import("palette.zig").DB16;

pub const CONF = struct {
    pub const VERSION = "alpha11";
    pub const THE_NAME = "P1Xel Tool";
    pub const WELCOME = "Create retro art for P1X x86 assembly engine.";
    pub const SCREEN_W = 1440;
    pub const SCREEN_H = 960;
    pub const PALETTES_FILE = "palettes.dat";
    pub const TILES_FILE = "tiles.dat";
    pub const PREVIEW_FILE = "preview.dat";
    pub const MAX_PALETTES = 128;
    pub const MAX_TILES = 128;
    pub const FONT_WIDTH = 8;
    pub const FONT_HEIGHT = 8;
    pub const FONT_DEFAULT_SIZE = 2;
    pub const FONT_BIG = 8;
    pub const FONT_SMOL = 1;
    pub const CORNER_RADIUS = 0.1;
    pub const CORNER_QUALITY = 2;
    pub const SHADOW = 4;
    pub const FRAME = 2;
    pub const SPRITE_SIZE = 16; // The actual sprite dimensions (16x16 pixels)
    pub const GRID_SIZE = 32; // How large each pixel appears on screen
    pub const CANVAS_SIZE = SPRITE_SIZE * GRID_SIZE;
    pub const CANVAS_FRAME_SIZE = 16;
    pub const CANVAS_X = 64;
    pub const CANVAS_Y = 64;
    pub const POPUP_BG_ALPHA = DB16.BLACK;
    pub const COLOR_BG = 0x021C35;
    pub const COLOR_PRIMARY = DB16.WHITE;
    pub const COLOR_SECONDARY = DB16.LIGHT_GRAY;
    pub const COLOR_CROSSHAIR = DB16.LIGHT_GRAY;
    pub const COLOR_YES = DB16.GREEN;
    pub const COLOR_NO = DB16.RED;
    pub const COLOR_OK = DB16.DARK_GRAY;
    pub const COLOR_POPUP = DB16.NAVY_BLUE;
    pub const COLOR_POPUP_MSG = DB16.WHITE;
    pub const COLOR_SHADOW = DB16.BLACK;
    pub const COLOR_LIGHT = DB16.WHITE;
    pub const COLOR_NAV_BG = 0x27345E;
    pub const COLOR_NAV_FRAME = DB16.BLACK;
    pub const COLOR_MENU_TEXT = 0x7DB9DD;
    pub const COLOR_MENU_FRAME = 0x27345E;
    pub const COLOR_MENU_FRAME_HOVER = DB16.WHITE;
    pub const COLOR_MENU_NORMAL = 0x274676;
    pub const COLOR_MENU_SECONDARY = 0x1B1C42;
    pub const COLOR_MENU_HIGHLIGHT = 0xe7e19d;
    pub const COLOR_MENU_DANGER = 0xe46b85;
    pub const VFX_SNOW_MIN = 4;
    pub const VFX_SNOW_MAX = 64;
    pub const VFX_SNOW_COLOR = 0x022546;
    pub const VFX_SNOW_SPEED_MIN = 50.0;
    pub const VFX_SNOW_SPEED_MAX = 500.0;
    pub const MAX_PREVIEW_H = 48;
    pub const MAX_PREVIEW_W = 32;
    pub const PREVIEW_W = 14;
    pub const PREVIEW_H = 20;
    pub const PREVIEW_LAYERS = 3;
    pub const PREVIEW_SCALE = 4;
    pub const PREVIEW_SIZE = SPRITE_SIZE * 4;
    pub const PALETTES_PER_PAGE = 36;
};
