...
pub const ENGINE_EXPORT_PATH = CONF.GBC_EXPORT_BINARY_PATH;
pub const ENGINE_EXPORT_INC_DIR = CONF.GBC_EXPORT_INC_DIR;
pub const ENGINE_EXPORT_INC_PATH = CONF.GBC_EXPORT_INC_PATH;
...
const OBJ_TILE_SLOTS: u8 = CONF.MAX_TILES;
const BG_TILE_BASE: u8 = OBJ_TILE_SLOTS;
...
pub fn exportGameBoyEngine(project: *const Project) !void {
    ...
    const export_root = resolveExportRoot(&root_buf);

    var export_dir_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const export_dir = try exportPath(&export_dir_buf, export_root, CONF.GBC_EXPORT_PROJECT_DIR);
    var binary_path_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const binary_path = try exportPath(&binary_path_buf, export_root, ENGINE_EXPORT_PATH);
    var inc_dir_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const inc_dir = try exportPath(&inc_dir_buf, export_root, ENGINE_EXPORT_INC_DIR);
    var inc_path_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const inc_path = try exportPath(&inc_path_buf, export_root, ENGINE_EXPORT_INC_PATH);

    try ensureExportDir(export_dir);
    try ensureExportDir(inc_dir);
    ...
    {
        var writer = try BinaryWriter.create(binary_path);
        defer writer.close();
...
fn writeHeader(writer: *BinaryWriter, project: *const Project) !void {
    const sprite_count = exportSpriteCount(project);
...
fn writeLevel(writer: *BinaryWriter, project: *const Project, level: ExportLevel) !void {
    ...
    try writeLogicMap(writer, project, map);
...
fn writeLogicMap(writer: *BinaryWriter, project: *const Project, map: *const Map) !void {
    var y: u16 = 0;
    while (y < ENGINE_MAP_H) : (y += 1) {
        var x: u16 = 0;
        while (x < ENGINE_MAP_W) : (x += 1) {
            const value: u8 = if (x < map.width and y < map.height) value: {
                const idx = @as(usize, y) * @as(usize, map.width) + x;
                break :value project.tileFlags(map.tile_ids[idx]);
            } else 0;
            try writer.writeU8(value);
        }
    }
...
fn exportTileCount(project: *const Project) u16 {
    return @min(project.imageCountMode(.tiles), @as(u16, CONF.MAX_TILES), MAX_BG_TILES);
}

fn exportSpriteCount(project: *const Project) u16 {
    return @min(project.imageCountMode(.sprites), @as(u16, OBJ_TILE_SLOTS));
}

fn writeRgbdsInclude(project: *const Project, inc_path: []const u8) !void {
    ...
    const logicmap_bytes: usize = tilemap_bytes;

    var writer = try TextWriter.create(inc_path);
    defer writer.close();
    ...
    try writer.print("DEF P1XB_LOGICMAP_BYTES                 EQU {d}\n", .{logicmap_bytes});
    ...
    try writer.writeAll("DEF P1XB_DESERT_SPRITES_OFFSET          EQU P1XB_DESERT_LOGICMAP_OFFSET + P1XB_LOGICMAP_BYTES\n\n");
...
fn resolveExportRoot(buffer: *[std.Io.Dir.max_path_bytes]u8) []const u8 {
    if (exportProjectExistsAt(".")) return ".";

    const len = std.process.currentPath(std.Options.debug_io, buffer) catch return ".";
    var candidate: []const u8 = buffer[0..len];
    while (candidate.len > 0) {
        if (exportProjectExistsAt(candidate)) return candidate;
...
fn exportProjectExistsAt(root: []const u8) bool {
    var path_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const project_dir = exportPath(&path_buf, root, CONF.GBC_EXPORT_PROJECT_DIR) catch return false;
    var dir = std.Io.Dir.cwd().openDir(std.Options.debug_io, project_dir, .{}) catch return false;
    dir.close(std.Options.debug_io);
    return true;
}

fn exportPath(buffer: []u8, root: []const u8, relative_path: []const u8) ![]const u8 {
    if (std.Io.Dir.path.isAbsolute(relative_path) or root.len == 0 or std.mem.eql(u8, root, ".")) return relative_path;
    return std.fmt.bufPrint(buffer, "{s}" ++ std.Io.Dir.path.sep_str ++ "{s}", .{ root, relative_path }) catch error.ExportOpenFailed;
}

fn ensureExportDir(path: []const u8) !void {
...
const TextWriter = struct {
    ...
    fn create(path: []const u8) !TextWriter {
        const file = std.Io.Dir.cwd().createFile(std.Options.debug_io, path, .{ .truncate = true }) catch |err| {
            std.debug.print("[export] failed to open {s} for writing: {s}\n", .{ path, @errorName(err) });
            return error.ExportOpenFailed;
        };
    ...
    fn writeAll(self: *const TextWriter, bytes: []const u8) !void {
        self.file.writeStreamingAll(std.Options.debug_io, bytes) catch |err| {
            std.debug.print("[export] failed to write to {s}: {s}\n", .{ self.path, @errorName(err) });
...
const BinaryWriter = struct {
    ...
    fn create(path: []const u8) !BinaryWriter {
        const file = std.Io.Dir.cwd().createFile(std.Options.debug_io, path, .{ .truncate = true }) catch |err| {
            std.debug.print("[export] failed to open {s} for writing: {s}\n", .{ path, @errorName(err) });
            return error.ExportOpenFailed;
        };
