    ...
    while (!rl.windowShouldClose()) {
        ...
        switch (key) {
            ...
            rl.KeyboardKey.tab => {
                ...
                current_palette_index = (current_palette_index + 1) % PALETTES.len;
            ...
            rl.KeyboardKey.left_shift, rl.KeyboardKey.right_shift => {
                ...
                if (rl.isKeyDown(rl.KeyboardKey.tab)) {
                    if (current_palette_index == 0) {
                        current_palette_index = PALETTES.len - 1;
                    } else {
        ...
        for (0..SPRITE_SIZE) |y| {
            for (0..SPRITE_SIZE) |x| {
                ...
                const db16_idx = PALETTES[current_palette_index][idx];

                // Only skip if it's index 0 AND the first palette color is black (transparent)
                if (idx == 0 and PALETTES[current_palette_index][0] == 0) {
        ...
        // Show current palette name and controls
        rl.drawText("ACTIVE PALETTE", sx, sy, 20, DB16.BLUE);
        sy += 24;
        ...
        inline for (0..4) |i| {
            ...
            const db16_idx = PALETTES[current_palette_index][i];
            ...
            buf[0] = '1' + index;
            buf[1] = 0;
            if (i == 0 and PALETTES[current_palette_index][0] == 0) {
        ...
        for (0..16) |i| {
            ...
            for (PALETTES[current_palette_index]) |palette_color| {
                if (palette_color == i) {
                    is_in_palette = true;
                    break;
                }
